module Ankat.ViewModel.Operations

open System

open Ankat

let private party = Ankat.AppContent.party
type private P = Ankat.ViewModel.Product1
...
and RunOperationInfo( operation : Operation) = 
    ...
    member x.LoggingRecords : LoggingRecord list = 
        let ops = Operation.tree operation
        [   for op in ops do
                yield! (party.GetLogOperation op.FullName).LoggingRecords ]
